...
if __name__ == '__main__':
    ...
    print("----------------------------------------------------")
    print("----------------------------------------------------")
    print("ANALYSE DE SENTIMENTS SUR DES APPRECIATIONS D'ETUDIANTS")
    ...
    for i in range(num_lines):
        sentence = f.readline()
        nonEnglish_sentences.append(sentence)

    for sentence in nonEnglish_sentences:
        translated_sentence = translator.translate(sentence, dest='en').text
        vs = analyzer.polarity_scores(translated_sentence)
        print("{: <10}\t {} ".format(sentence,str(vs['compound'])))
        print("----------------------------------------------------")
